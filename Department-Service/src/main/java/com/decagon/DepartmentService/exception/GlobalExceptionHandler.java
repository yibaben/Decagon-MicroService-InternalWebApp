...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler(CodeNumAlreadyExistException.class)
    public ResponseEntity<ErrorDetails> CodeNumExceptionHandler(CodeNumAlreadyExistException exception,
                                                              WebRequest webRequest){
        ErrorDetails errorDetails = new ErrorDetails(
                LocalDateTime.now(),
                exception.getMessage(),
                webRequest.getDescription(false),
                "DEPARTMENT_CODE_NUMBER_ALREADY_EXIST"
        );
